					 ...
					 const char *buf, size_t count)
{
	...
	pr_info("%s: gpu max pwrlevel changed! %d\n", __func__, level);
	mutex_lock(&device->mutex);
...
#ifdef CONFIG_MACH_ACER_A12
static bool is_limit_mode_enable;
static int kgsl_pwrctrl_limit_mode_show(struct device *dev,
					struct device_attribute *attr,
					char *buf)
{
	return sprintf(buf, "%d", is_limit_mode_enable);
}

static int kgsl_pwrctrl_limit_mode_store(struct device *dev,
					 struct device_attribute *attr,
					 const char *buf, size_t count)
{
	int ret, enable;
	...
	pr_info("%s: limit mode:%d\n", __func__, enable);
	if (enable) {
		tmp_buf = GPU_LIMIT_MODE;
		is_limit_mode_enable = true;
	} else {
		tmp_buf = GPU_NORMAL_MODE;
		is_limit_mode_enable = false;
	}
	return kgsl_pwrctrl_max_pwrlevel_store(dev, attr, &tmp_buf, 1);
}
#endif
...
#ifdef CONFIG_MACH_ACER_A12
DEVICE_ATTR(limit_mode, 0644, kgsl_pwrctrl_limit_mode_show, kgsl_pwrctrl_limit_mode_store);
#endif
