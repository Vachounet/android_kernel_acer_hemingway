...
#define AD5816_REG_VCM_CODE_MSB 0x03
#define AD5816_REG_VCM_CODE_LSB 0x04

#define REGISTERSIZE 4
uint16_t af_register[REGISTERSIZE]={0x02,0x06,0x07,0x08};
uint16_t af_initsetting[REGISTERSIZE]={0x02,0x02,0x4A,0x46};
int af_init = 0;
#endif
	...
	int16_t next_lens_position, uint32_t hw_params, uint16_t delay)
{
	...
	if (write_arr[i].reg_write_type == MSM_ACTUATOR_WRITE_DAC){
		...
		ad5816_ring_ctrl = (hw_params & 0x100) >> 6;
		msb = ad5816_ring_ctrl | ((next_lens_position & 0x0300) >> 8);
		lsb = next_lens_position & 0x00FF;

		CDBG("%s ad5816_vcm_step_time:0x%x next_lens_position:%d, msb:0x%x lsb:0x%x\n",
		__func__, hw_params, next_lens_position, msb, lsb);

		rc = msm_camera_cci_i2c_write(&a_ctrl->i2c_client, AD5816_REG_VCM_CODE_MSB,
		...
		rc = msm_camera_cci_i2c_write(&a_ctrl->i2c_client, AD5816_REG_VCM_CODE_LSB,
...
#ifdef CONFIG_MACH_ACER_A12
	if(af_init == 1){
		pr_info("%s AF init ++",__func__);
		for(i=0 ;i<REGISTERSIZE ;i++) {
			CDBG("%s af_register = %x af_initsetting = %x",__func__,af_register[i],af_initsetting[i]);
			rc = msm_camera_cci_i2c_write(&a_ctrl->i2c_client, af_register[i],
			af_initsetting[i], MSM_CAMERA_I2C_BYTE_DATA);

			if(rc < 0) {
				pr_err("%s AF init fail, af_register = %x af_initsetting = %x",__func__,af_register[i],af_initsetting[i]);
				return rc;
			}
		}
		af_init = 0;
