...
#ifdef CONFIG_ARCH_ACER_MSM8974
static struct dentry *dfs_pmic_dir;
static struct dentry *dfs_pmic_sleep_dir;
#endif
...
#ifdef CONFIG_ARCH_ACER_MSM8974
#define PM8941_NR_GPIOS 36
#define PM8941_NR_MPPS 8
#define PM8841_NR_MPPS 4

struct pm8941_gpio_regs {
	u8 mode[PM8941_NR_GPIOS];
	u8 state[PM8941_NR_GPIOS];
};
struct pm8941_mpp_regs {
	u8 mode[PM8941_NR_MPPS];
	u8 state[PM8941_NR_MPPS];
};
struct pm8841_mpp_regs {
	u8 mode[PM8841_NR_MPPS];
	u8 state[PM8841_NR_MPPS];
};

static struct pm8941_gpio_regs pm8941_gpio_sleep_state = {{0}, {0}};
static struct pm8941_mpp_regs pm8941_mpp_sleep_state = {{0}, {0}};
static struct pm8841_mpp_regs pm8841_mpp_sleep_state = {{0}, {0}};
...
void save_pmic_gpio_mpp_sleep_state(void)
{
	...
	list_for_each_entry(q_chip, &qpnp_pin_chips, chip_list) {
		gpio_chip = &q_chip->gpio_chip;
		if(!strcmp(q_chip->gpio_chip.label, "pm8941-gpio")){
			for (i = 0; i < gpio_chip->ngpio; i++) {
				q_spec = qpnp_chip_gpio_get_spec(q_chip, i);
				pm8941_gpio_sleep_state.mode[i] = ((q_spec->regs[Q_REG_I_MODE_CTL] & Q_REG_MODE_SEL_MASK) >> Q_REG_MODE_SEL_SHIFT);
				pm8941_gpio_sleep_state.state[i] = qpnp_pin_get(gpio_chip, i);
			}
		} else if(!strcmp(q_chip->gpio_chip.label, "pm8941-mpp")){
			for (i = 0; i < gpio_chip->ngpio; i++) {
				q_spec = qpnp_chip_gpio_get_spec(q_chip, i);
				pm8941_mpp_sleep_state.mode[i] = ((q_spec->regs[Q_REG_I_MODE_CTL] & Q_REG_MODE_SEL_MASK) >> Q_REG_MODE_SEL_SHIFT);
				pm8941_mpp_sleep_state.state[i] = qpnp_pin_get(gpio_chip, i);
			}
		} else if(!strcmp(q_chip->gpio_chip.label, "pm8841-mpp")){
			for (i = 0; i < gpio_chip->ngpio; i++) {
				q_spec = qpnp_chip_gpio_get_spec(q_chip, i);
				pm8841_mpp_sleep_state.mode[i] = ((q_spec->regs[Q_REG_I_MODE_CTL] & Q_REG_MODE_SEL_MASK) >> Q_REG_MODE_SEL_SHIFT);
				pm8841_mpp_sleep_state.state[i] = qpnp_pin_get(gpio_chip, i);
			}
		}
	}
	return;
}
EXPORT_SYMBOL(save_pmic_gpio_mpp_sleep_state);

static int dbg_pmic_gpio_sleep_show(struct seq_file *s, void *unused)
{
	struct qpnp_pin_chip *q_chip = s->private;
	struct gpio_chip *gpio_chip;
	struct qpnp_pin_spec *q_spec;
	u8 mode, state;
	int i;
	const char *label;
	static const char * const gpio_mode[] = { "in", "out", "in/out", "reserved"};
	static const char * const mpp_mode[] = {	"d_in", "d_out", "d_in/d_out", "bi_dir",
						"a_in", "a_out", "sink", "reserved"};
	gpio_chip = &q_chip->gpio_chip;
	...
	if(!strcmp(q_chip->gpio_chip.label, "pm8941-gpio")){
		for (i = 0; i < gpio_chip->ngpio; i++) {
			...
			mode = pm8941_gpio_sleep_state.mode[i];
			state = pm8941_gpio_sleep_state.state[i];
		...
		for (i = 0; i < gpio_chip->ngpio; i++) {
			q_spec = qpnp_chip_gpio_get_spec(q_chip, i);
			label = gpiochip_is_requested(gpio_chip, i);
			mode = pm8941_mpp_sleep_state.mode[i];
			state = pm8941_mpp_sleep_state.state[i];
		...
		for (i = 0; i < gpio_chip->ngpio; i++) {
			q_spec = qpnp_chip_gpio_get_spec(q_chip, i);
			label = gpiochip_is_requested(gpio_chip, i);
			mode = pm8841_mpp_sleep_state.mode[i];
			state = pm8841_mpp_sleep_state.state[i];
			seq_printf(s, "gpio-%-3d (mpp-%-2d) (%-12.12s) %-10.10s"
					" %s\n",
					gpio_chip->base + i,
					i + 1,
...
static int dbg_pmic_gpio_show(struct seq_file *s, void *unused)
{
	struct qpnp_pin_chip *q_chip = s->private;
	struct gpio_chip *gpio_chip;
	struct qpnp_pin_spec *q_spec;
	...
	for (i = 0; i < gpio_chip->ngpio; i++) {
		q_spec = qpnp_chip_gpio_get_spec(q_chip, i);
		label = gpiochip_is_requested(gpio_chip, i);
		mode = ((q_spec->regs[Q_REG_I_MODE_CTL] & Q_REG_MODE_SEL_MASK) >> Q_REG_MODE_SEL_SHIFT);
...
static int dbg_pmic_gpio_sleep_open(struct inode *inode, struct file *file)
{
	struct qpnp_pin_chip *q_chip = (struct qpnp_pin_chip *)inode->i_private;
	return single_open(file, dbg_pmic_gpio_sleep_show, q_chip);
}

static const struct file_operations pmic_gpio_sleep_debug_fops = {
	.open           = dbg_pmic_gpio_sleep_open,
	.read           = seq_read,
	.llseek         = seq_lseek,
	.release        = single_release,
};

static int dbg_pmic_gpio_open(struct inode *inode, struct file *file)
{
	struct qpnp_pin_chip *q_chip = (struct qpnp_pin_chip *)inode->i_private;
	return single_open(file, dbg_pmic_gpio_show, q_chip);
}

static const struct file_operations pmic_gpio_debug_fops = {
	.open           = dbg_pmic_gpio_open,
	.read           = seq_read,
	.llseek         = seq_lseek,
	.release        = single_release,
};

static int pmic_info_debugfs_create(struct qpnp_pin_chip *q_chip)
{
	struct spmi_device *spmi = q_chip->spmi;
	...
	dfs = debugfs_create_file(
					...
					dfs_pmic_sleep_dir,
					q_chip,
					&pmic_gpio_sleep_debug_fops);
	if (dfs == NULL)
		goto dfs_err;

	return 0;
dfs_err:
	dev_err(dev, "%s: cannot register debugfs for pmic info\n",
			__func__);
	debugfs_remove_recursive(dfs_pmic_dir);
	return -ENFILE;
}
...
#ifdef CONFIG_ARCH_ACER_MSM8974
	rc = pmic_info_debugfs_create(q_chip);
	if (rc) {
		dev_err(&spmi->dev, "%s: debugfs creation failed\n", __func__);
		goto err_probe;
	}
...
#ifdef CONFIG_ARCH_ACER_MSM8974
	dfs_pmic_dir = debugfs_create_dir("pmic_gpio_mpp_state", NULL);
	if (dfs_pmic_dir == NULL)
		pr_err("Cannot register top level debugfs directory\n");
	dfs_pmic_sleep_dir = debugfs_create_dir("pmic_gpio_mpp_sleep_state", NULL);
	if (dfs_pmic_sleep_dir == NULL)
		pr_err("Cannot register top level debugfs directory\n");
#endif
