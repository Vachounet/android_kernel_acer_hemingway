...
#define CY_CORE_MODE_CHANGE_TIMEOUT		1500
#define CY_CORE_RESET_AND_WAIT_TIMEOUT		1500
...
static int cyttsp4_startup_(struct cyttsp4_core_data *cd)
{
	...
	cyttsp4_stop_wd_timer(cd);
	printk("\n [cyttsp4_startup] check gpio 0 :%d\n", gpio_get_value(0));
...
static int cyttsp4_core_probe(struct cyttsp4_core *core)
{
	...
	pdata->irq_gpio = ts_gpio_interrupt;
	printk("\n[Cyttsp4_core][probe]pdata->irq_gpio :%d\n",pdata->irq_gpio);

	cd->irq = gpio_to_irq(pdata->irq_gpio);
	printk(" \n [cyttsp4_core_probe] pdata->irq_gpio:%d, cd->irq :%d, \n",pdata->irq_gpio, cd->irq );
	cd->irq_enabled = true;
	...
	if (cd->pdata->init) {
		dev_info(cd->dev, "%s: cfg gpio\n", __func__);
		rc = cd->pdata->init(cd->pdata, 1, cd->dev);
	} else {
		dev_info(cd->dev, "%s: No cfg gpio function\n", __func__);
		rc = 0;
	}
	if (rc < 0)
		dev_err(cd->dev, "%s: cfg gpio fail r=%d\n", __func__, rc);

	if (cd->pdata->hw_init) {
		dev_info(cd->dev, "%s: Init HW\n", __func__);
		/*TODO : check why cannot config*/
		//rc = cd->pdata->hw_init(cd->pdata, 1, dev);//cd->dev);
	} else {
	...
	rc = request_threaded_irq(cd->irq, NULL, cyttsp4_irq, irq_flags,
		dev_name(dev), cd);
	if (rc < 0) {
		dev_err(dev, "%s: Error, could not request irq, rc:%d\n", __func__, rc);
