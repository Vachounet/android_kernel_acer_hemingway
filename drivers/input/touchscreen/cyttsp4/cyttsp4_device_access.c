		...
		struct device_attribute *attr, char *buf)
{
	...
	do {
		u8 val_h = return_buf[CY_CMD_RET_PNL_OUT_ELMNT_SZ_OFFS_H];
		u8 val_l = return_buf[CY_CMD_RET_PNL_OUT_ELMNT_SZ_OFFS_L];
		returnedElement = val_h * 256 + val_l;
		leftOverElement -= returnedElement;
		/* get the data */
		rc = _cyttsp4_ret_scan_data_cmd(dev, returnedElement,
