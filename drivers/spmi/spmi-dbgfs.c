...
#define PM8841_SMPS_ENABLE_BIT 7
#define PM8841_SMPS_COUNT 8

#define PM8941_LDO_REG_BASE 0x14000
#define PM8941_VS_REG_BASE 0x18000
#define PM8941_VS_ENABLE_BIT 7
#define PM8941_LDO_ENABLE_OFFSET 0x9
#define PM8941_LDO_ENABLE_BIT 5
#define PM8941_LDO_VSEL_OFFSET 0x41
#define PM8941_SMPS_BOOST_COUNT 9
#define PM8941_LDO_COUNT 24
...
#define VREG(_name, _enable_addr, _vsel_addr) \
	{ \
		.name	= _name, \
		.enable_addr	= _enable_addr, \
		.vsel_addr	= _vsel_addr, \
	}

static struct pmic_reg pm8941_reg[] = {
	VREG("S1",  0x11446, 0x11441),
	VREG("S2",  0x11746, 0x11741),
	VREG("S3",  0x11A46, 0x11A41),
	VREG("LVS1",  0x18046, 0),
	VREG("LVS2", 0x18146, 0),
	VREG("LVS3", 0x18246, 0),
	VREG("5VS1",  0x18346, 0),
	VREG("5VS2", 0x18446, 0),
	VREG("5V_BOOST",  0x1A046, 0x1A041),
};

static struct pmic_reg pm8841_reg[] = {
	VREG("S1", 0x51446, 0x51441),
	VREG("S2", 0x51746, 0x51741),
	VREG("S3", 0x51A46, 0x51A41),
	VREG("S4", 0x51D46, 0x51D41),
	VREG("S5", 0x52046, 0x52041),
	VREG("S6", 0x52346, 0x52341),
	VREG("S7", 0x52646, 0x52641),
	VREG("S8", 0x52946, 0x52941),
};

struct spmi_ctrl_data *spmi0_ctrl_data;
static struct pmic_status pm8941_sleep_status[PM8941_SMPS_BOOST_COUNT + PM8941_LDO_COUNT];
static struct pmic_status pm8841_sleep_status[PM8841_SMPS_COUNT];
...
#ifdef CONFIG_ARCH_ACER_MSM8974
int save_pmic_sleep_vreg_state(void)
{
	int i;
	u8  data[ITEMS_PER_LINE];

	/* PM8941 SMPS_BOOST */
	for(i = 0 ; i < PM8941_SMPS_BOOST_COUNT; i++){
		strcpy(pm8941_sleep_status[i].name, pm8941_reg[i].name);
		if(pm8941_reg[i].enable_addr != 0){
			spmi_read_data(spmi0_ctrl_data->ctrl, data, pm8941_reg[i].enable_addr, 1);

			if((data[0] & (1 << PM8941_VS_ENABLE_BIT)))
				strcpy(pm8941_sleep_status[i].enable, "ON");
			else
				strcpy(pm8941_sleep_status[i].enable, "OFF");
		}
		else
			strcpy(pm8941_sleep_status[i].enable, "NA");
	}

	/* PM8941 LDO */
	for(i = 0; i < PM8941_LDO_COUNT; i++){
		sprintf(pm8941_sleep_status[i + PM8941_SMPS_BOOST_COUNT].name, "LDO%d", i+1);
		spmi_read_data(spmi0_ctrl_data->ctrl, data, PM8941_LDO_REG_BASE + PM8941_LDO_ENABLE_OFFSET + (0x100 * i), 1);

		if((data[0] & (1 << PM8941_LDO_ENABLE_BIT)))
			strcpy(pm8941_sleep_status[i + PM8941_SMPS_BOOST_COUNT].enable, "ON");
		else
			strcpy(pm8941_sleep_status[i + PM8941_SMPS_BOOST_COUNT].enable, "OFF");
	}

	/* PM8841 SMPS  */
	for(i = 0 ; i < PM8841_SMPS_COUNT; i++){
		strcpy(pm8841_sleep_status[i].name, pm8841_reg[i].name);
		spmi_read_data(spmi0_ctrl_data->ctrl, data, pm8841_reg[i].enable_addr, 1);

		if((data[0] & (1 << PM8841_SMPS_ENABLE_BIT)))
			strcpy(pm8841_sleep_status[i].enable, "ON");
		else
			strcpy(pm8841_sleep_status[i].enable, "OFF");
	}

	return 1;
}
EXPORT_SYMBOL(save_pmic_sleep_vreg_state);


static int dump_pmic_vreg(struct spmi_trans *trans)
{
	int i;
	u8  data[ITEMS_PER_LINE];
	struct spmi_log_buffer *log = trans->log;

	print_to_log(log, "PM8941 VREG State:\n");
	print_to_log(log, "--------------------------------------------------------------------------\n");
	print_to_log(log, "   NAME   ENABLE  VSEL\n");

	/* PM8941 SMPS_BOOST */
	for(i=0 ; i < PM8941_SMPS_BOOST_COUNT; i++){
		print_to_log(log, "%8s : ", pm8941_reg[i].name);
		if(pm8941_reg[i].enable_addr != 0){
			trans->addr = pm8941_reg[i].enable_addr;
			spmi_read_data(trans->ctrl, data, trans->addr, 1);
			if((data[0] & (1 << PM8941_VS_ENABLE_BIT)))
				print_to_log(log, "ON     ");
			else
				print_to_log(log, "OFF    ");
		}
		else
			print_to_log(log, "       ");

		if(pm8941_reg[i].vsel_addr != 0){
			trans->addr = pm8941_reg[i].vsel_addr;
			spmi_read_data(trans->ctrl, data, trans->addr, 1);
			print_to_log(log, "0x%2.2X\n", data[0]);
		}
		else
			print_to_log(log, "\n");
	}
	...
	/* PM8941 LDO */
	for(i=0 ; i < PM8941_LDO_COUNT; i++){
		trans->addr = PM8941_LDO_REG_BASE + PM8941_LDO_ENABLE_OFFSET + (0x100 * i);

		print_to_log(log, "   LDO%2i : ", i+1);
		...
		trans->addr = PM8941_LDO_REG_BASE + PM8941_LDO_VSEL_OFFSET + (0x100 * i);
	...
	/* PM8841 SMPS */
	for(i=0 ; i < PM8841_SMPS_COUNT; i++){
		print_to_log(log, "%8s : ", pm8841_reg[i].name);
		trans->addr = pm8841_reg[i].enable_addr;
		spmi_read_data(trans->ctrl, data, trans->addr, 1);
		if((data[0] & (1 << PM8841_SMPS_ENABLE_BIT)))
			print_to_log(log, "ON     ");
		else
		...
		trans->addr = pm8841_reg[i].vsel_addr;
		spmi_read_data(trans->ctrl, data, trans->addr, 1);
		print_to_log(log, "0x%2.2X\n", data[0]);
	}
...
static int dump_pmic_sleep_vreg(struct spmi_trans *trans)
{
	int i;
	struct spmi_log_buffer *log = trans->log;
	...
	print_to_log(log, "PM8941 VREG State:\n");
	print_to_log(log, "--------------------------------------------------------------------------\n");
	print_to_log(log, "   NAME    ENABLE\n");

	for(i = 0; i < PM8941_SMPS_BOOST_COUNT + PM8941_LDO_COUNT; i++){
		print_to_log(log, "%8s : %s\n", pm8941_sleep_status[i].name, pm8941_sleep_status[i].enable);
	}
	...
	for(i = 0; i < PM8841_SMPS_COUNT; i++){
		print_to_log(log, "%8s : %s\n", pm8841_sleep_status[i].name, pm8841_sleep_status[i].enable);
	}
...
static ssize_t spmi_dfs_pmic_status_read(struct file *file, char __user *buf,
	size_t count, loff_t *ppos)
{
	struct spmi_trans *trans = file->private_data;
	struct spmi_log_buffer *log = trans->log;
	...
	if (log->rpos == 0 && log->wpos == 0)
		dump_pmic_vreg(trans);
	else if (log->rpos >= log->wpos)
		return 0;
...
static ssize_t spmi_dfs_pmic_sleep_status_read(struct file *file, char __user *buf,
	size_t count, loff_t *ppos)
{
	struct spmi_trans *trans = file->private_data;
	struct spmi_log_buffer *log = trans->log;
...
#ifdef CONFIG_ARCH_ACER_MSM8974
static const struct file_operations spmi_dfs_pmic_status_fops = {
	.open		= spmi_dfs_data_open,
	.release	= spmi_dfs_close,
	.read		= spmi_dfs_pmic_status_read,
};

static const struct file_operations spmi_dfs_pmic_sleep_status_fops = {
	.open		= spmi_dfs_data_open,
	.release	= spmi_dfs_close,
	.read		= spmi_dfs_pmic_sleep_status_read,
};
#endif
...
#ifdef CONFIG_ARCH_ACER_MSM8974
	if(!strcmp(ctrl->dev.kobj.name, "spmi-0")){
		spmi0_ctrl_data = ctrl_data;

		file = debugfs_create_file("pmic_vreg_status", DFS_MODE, dir, ctrl_data,
							&spmi_dfs_pmic_status_fops);
		if (!file) {
			pr_err("error creating 'pmic_vreg_status' entry\n");
			goto err_remove_fs;
		}

		file = debugfs_create_file("pmic_vreg_sleep_status", DFS_MODE, dir, ctrl_data,
							&spmi_dfs_pmic_sleep_status_fops);
		if (!file) {
			pr_err("error creating 'pmic_vreg_sleep_status' entry\n");
			goto err_remove_fs;
		}
	}
#endif
