...
#define PM8941_SID	0
#define PM8941_REG_HWREV	0x103	/* PM8941 REVID_REVISION4 */
#define PM8941_REG_SUBTYPE	0x105	/* PM8941 REVID_PERPH_SUBTYPE */
u8 revision, subtype;
...
static int __devinit spmi_pmic_arb_probe(struct platform_device *pdev)
{
	...
	ret = sysfs_create_group(dev_info_pmic_kobj, &pmic_attr_group);
	if(ret) {
		pr_err("Failed to create dev-info_pmic sysfs group\n");
	}

	if(pmic_arb_read_cmd(&pmic_arb->controller,
				SPMI_CMD_EXT_READL, PM8941_SID, PM8941_REG_SUBTYPE, 1, &subtype))
		dev_err(&pdev->dev, "Failed to get subtype of PMIC chip\n");

	if(pmic_arb_read_cmd(&pmic_arb->controller,
				SPMI_CMD_EXT_READL, PM8941_SID, PM8941_REG_HWREV, 1, &revision))
		dev_err(&pdev->dev, "Failed to get revision of PMIC chip\n");
#endif
