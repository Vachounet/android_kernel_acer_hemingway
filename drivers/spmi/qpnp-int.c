...
#define SID_PM8941 0
#define PID_PON 8
#define PID_USB_IN 19
#define PID_RTC_ALARM 97
...
#ifdef CONFIG_ARCH_ACER_MSM8974
static int woken_by_pwr_key;
int is_woken_by_pwr_key(void)
{
	return woken_by_pwr_key;
}
void clean_woken_by_pwr_key(void)
{
	woken_by_pwr_key = 0;
}
...
static void qpnp_show_resume_irq(uint8_t slave_id, uint8_t per_id, uint8_t irq_id)
{
	if(slave_id == SID_PM8941 && per_id == PID_PON) {
		if(!woken_by_pwr_key)
			woken_by_pwr_key = 1;
		pr_warning("Resume by PM8941 PON\n");
	}
	else if(slave_id == SID_PM8941 && per_id == PID_USB_IN)
		pr_warning("Resume by PM8941 USB_IN\n");
	else if(slave_id == SID_PM8941 && per_id == PID_RTC_ALARM)
		pr_warning("Resume by PM8941 RTC_ALARM\n");
	else
		pr_warning("Resume by unknown reason\n");

	return;
}

void qpnp_resume_by_pmic(void)
{
	wake_by_pmic = 1;
}
EXPORT_SYMBOL(qpnp_resume_by_pmic);
#endif
...
#ifdef CONFIG_ARCH_ACER_MSM8974
	if(wake_by_pmic == 1){
		qpnp_show_resume_irq(spec->slave, spec->per, spec->irq);
		wake_by_pmic = 0;
	}
#endif

	busno = spmi_ctrl->nr;
	if (!spmi_ctrl || busno >= QPNPINT_MAX_BUSSES)
		return -EINVAL;
