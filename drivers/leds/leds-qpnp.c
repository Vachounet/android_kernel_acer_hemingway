...
#define WLED_CABC_EN_REG(base, n)    (WLED_MOD_EN_REG(base, n) + 0x06)
...
#ifdef CONFIG_MACH_ACER_A12
	if (level != 0 && level != WLED_MAX_DUTY_CYCLE)
		pr_info("%s::wled backlight adjust-%d\n", __func__, level);
#endif
...
static int __devinit qpnp_wled_init(struct qpnp_led_data *led)
{
	...
	if (led->wled_cfg->cs_out_en) {
		if (num_wled_strings == 1) {
			en_current_sink = 1;
		} else if (num_wled_strings == 2) {
			en_current_sink = 3;
		} else {
			en_current_sink = 7;
		}
	...
	/* set to default value, 1.2Mhz, 12bit, analog dimming */
	rc = qpnp_led_masked_write(led, WLED_MOD_SCHEME_REG(led->base),
					0x03, 0x00);
	if (rc) {
		dev_err(&led->spmi_dev->dev,
			"WLED mod sheme reg write failed(%d)\n", rc);
	}
	...
	for (i = 0; i < num_wled_strings; i++) {
		...
		if (led->wled_cfg->cabc_en) {
			rc = qpnp_led_masked_write(led, WLED_CABC_EN_REG(led->base, i),
					WLED_NO_MASK, WLED_CABC_EN);
			if (rc) {
				dev_err(&led->spmi_dev->dev,
				...
				struct device_node *node)
{
	...
	if (acer_boot_mode == CHARGER_BOOT) {
		led->default_on = false;
		pr_info("charging boot, disable backlight during booting.");
	}
#endif
