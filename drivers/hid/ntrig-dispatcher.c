...
static void clean_touch_flag_work_func(struct work_struct *work);
static DECLARE_DELAYED_WORK(touch_event_work, clean_touch_flag_work_func);
static struct workqueue_struct *touch_event_wq;
static int touch_event_flag;
#endif

#if defined(CONFIG_ARCH_ACER_MSM8974)
int no_touch_event(void)
{
...
void ntrig_send_sync(void){
	struct input_dev *input = ntrig_devices[0].multi_touch_device;
	int missed = gCurrentFingers;
	int j;
	if(input != NULL){
		if (missed) {
			for (j = 0; missed; j++) {
		...
		printk("input device is not null, sending input sync\n");
...
{
	int i;
	//int A12_PANEL_WIDTH= 1080;// for factory 1 st
...
#if defined(CONFIG_ARCH_ACER_MSM8974)
	touch_event_flag = 1;
	queue_delayed_work(touch_event_wq, &touch_event_work, msecs_to_jiffies(2000));
#endif
	...
	struct mr_message_types_s *multi_touch)
{
	...
	for (i = 0; i < fingers_num; i++) {
		...
		if (fingers[i].removed) {
			...
			/*[33953]Work around solution to avoid false trigger in top-left side*/
			if( (fingers[i].x_coord == 0) || (fingers[i].x_coord == 7200)){
				continue;
			}
...
#if defined(CONFIG_ARCH_ACER_MSM8974)
	touch_event_wq = create_singlethread_workqueue("touch_event_wq");
	if (!touch_event_wq) {
		printk("[ntrig-dispatcher] Not enough to alloc workqueue.\n");
		return -ENOMEM;
	}
#endif
