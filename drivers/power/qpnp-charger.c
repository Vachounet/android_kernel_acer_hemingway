...
static bool is_charger_unstable = false;
#endif
...
qpnp_chg_usb_usbin_valid_irq_handler(int irq, void *_chip)
{
	...
	if (chip->usb_present ^ usb_present) {
		...
		if (ts.tv_sec - last_ts.tv_sec >= 0 &&
			ts.tv_sec - last_ts.tv_sec <= 2) {
			long interval_ms = (ts.tv_sec - last_ts.tv_sec)*1000
						+ (ts.tv_nsec - last_ts.tv_nsec)/1000000;
			if (interval_ms < 200) {
				pr_info("charger unstable (%ldms)\n", interval_ms);
				is_charger_unstable = true;
			} else {
				is_charger_unstable = false;
			}
		} else {
			is_charger_unstable = false;
		}
...
#ifdef CONFIG_MACH_ACER_A12
		/* if USB current bigger than 500mA, show charging(AC) */
...
#ifdef CONFIG_MACH_ACER_A12
	if (!get_prop_batt_present(chip)) {
		if (get_prop_battery_voltage_now(chip)/1000 <= 2800) {
			pr_info("battery is missing\n");
			as3668_heat_indicator(BATT_BAD);
...
#ifdef CONFIG_MACH_ACER_A12
			/* Set input current to 1000mA while AC is unstable*/
			if (is_charger_unstable && ret.intval > 1000000)
				ret.intval = 1000000;
#endif
...
#define POWER_KEY_CHECK_MS 5000
static void
qpnp_power_key_work(struct work_struct *work)
{
	...
	if (qpnp_pon_is_pwrkey_press() == 1) {
		...
		if (chip->charging_disabled) {
			chip->charging_disabled = false;
			/* If vbat > vbatdet, using resumeing charging to enable charging. */
			if (qpnp_chg_is_usb_chg_plugged_in(chip)) {
				u8 chgr_sts;
				if (qpnp_chg_read(chip, &chgr_sts, INT_RT_STS(chip->chgr_base), 1))
					pr_err("failed to read interrupt sts\n");
				if (!chip->resuming_charging && (chgr_sts & VBAT_DET_LOW_IRQ)) {
					chip->resuming_charging = true;
					qpnp_chg_set_appropriate_vbatdet(chip);
					pr_info("resuming charging\n");
				}
			}
			qpnp_chg_charge_en(chip, !chip->charging_disabled);
			qpnp_chg_force_run_on_batt(chip, 0);
