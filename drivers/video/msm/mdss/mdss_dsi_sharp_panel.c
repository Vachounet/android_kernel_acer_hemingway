...
{
	if (!p5v_en)
		pr_debug("%s:%d, p5v_en line not configured\n",
			   __func__, __LINE__);

	if (!n5v_en)
		pr_debug("%s:%d, n5v_en line not configured\n",
			   __func__, __LINE__);
	...
	if (enable) {
		/* It work before seding commands for LP11 */
	} else {
		gpio_set_value(rst_gpio, 0);
		msleep(2);
		wmb();
		gpio_set_value(n5v_en, 0);
		msleep(2);
		wmb();
		gpio_set_value(p5v_en, 0);
		msleep(100);
		wmb();
	}
}
...
static int mdss_dsi_panel_on(struct mdss_panel_data *pdata)
{
	...
	if (mipi->mode == DSI_VIDEO_MODE) {
		/* LP11-low power high high */
		gpio_set_value(p5v_en, 1);
		wmb();
		gpio_set_value(n5v_en, 1);
		msleep(2);
		wmb();
			       ...
			       char *bl_ctrl)
{
	...
	/* lcdc_p5v_enable */
	p5v_en = of_get_named_gpio(np, "qcom,enable-p5v-gpio", 0);
	if (!gpio_is_valid(p5v_en)) {
		pr_err("%s:%d, p5v_en gpio not specified\n",
						__func__, __LINE__);
		return -ENODEV;
	}

	rc = gpio_request(p5v_en, "p5v_enable");
	if (rc) {
		pr_err("request p5v_en gpio failed, rc=%d\n",
			rc);
		gpio_free(p5v_en);
		return -ENODEV;
	}
	rc = gpio_direction_output(p5v_en, 0);
	if (rc) {
		pr_err("set_direction for p5v_en gpio failed, rc=%d\n",
			rc);
		gpio_free(p5v_en);
		return -ENODEV;
	}
	...
	/* lcdc_n5v_enable */
	n5v_en = of_get_named_gpio(np, "qcom,enable-n5v-gpio", 0);
	if (!gpio_is_valid(n5v_en)) {
		pr_err("%s:%d, n5v_en gpio not specified\n",
						__func__, __LINE__);
		return -ENODEV;
	}

	rc = gpio_request(n5v_en, "n5v_enable");
	if (rc) {
		pr_err("request n5v_en gpio failed, rc=%d\n",
			rc);
		gpio_free(n5v_en);
		gpio_free(p5v_en);
		return -ENODEV;
	}
	rc = gpio_direction_output(n5v_en, 0);
	if (rc) {
		pr_err("set_direction for n5v_en gpio failed, rc=%d\n",
			rc);
		gpio_free(n5v_en);
		gpio_free(p5v_en);
		return -ENODEV;
