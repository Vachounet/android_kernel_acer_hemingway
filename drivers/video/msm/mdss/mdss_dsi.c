...
/*
 *if the system is woken by power key, we need to set +5v earily.
 *For speedup resume time
 */
extern int is_woken_by_pwr_key(void);
extern void ctl_p5v(int gpio, int on_off);
extern void clean_woken_by_pwr_key(void);
#endif
...
static int mdss_dsi_panel_power_on(struct mdss_panel_data *pdata, int enable)
{
	...
	if (enable) {
		if (ctrl_pdata->power_data.num_vreg > 0) {
			...
			if (!regulator_is_enabled(ctrl_pdata->shared_pdata.vdd_io_vreg)) {
				  ...
				  int event, void *arg)
{
	...
	case MDSS_EVENT_RESUME:
		if (is_woken_by_pwr_key()) {
			pr_info("woken up by power key....");
			ctl_p5v(ctrl_pdata->p5v_gpio, 1);
		}
		clean_woken_by_pwr_key();
		break;
...
#ifdef CONFIG_MACH_ACER_A12
		ctrl_pdata->cont_splash_done = true;
#endif
		break;
			      ...
			      struct mdss_panel_common_pdata *panel_data)
{
...
#ifdef CONFIG_FB_MSM_MDSS_MIPI_DSI_INNOLUX
	ctrl_pdata->p5v_gpio = of_get_named_gpio(pdev->dev.of_node,
						 "qcom,p5v-gpio", 0);
	if (!gpio_is_valid(ctrl_pdata->p5v_gpio)) {
		pr_err("%s:%d, p5v gpio not specified\n",
						__func__, __LINE__);
	} else {
		rc = gpio_request(ctrl_pdata->p5v_gpio, "disp_p5v");
		if (rc) {
			pr_err("request reset gpio failed, rc=%d\n",
				rc);
			gpio_free(ctrl_pdata->rst_gpio);
		...
		if (cont_splash_enabled)
			gpio_direction_output(ctrl_pdata->p5v_gpio, 1);
		else
			gpio_direction_output(ctrl_pdata->p5v_gpio, 0);
	}

	ctrl_pdata->n5v_gpio = of_get_named_gpio(pdev->dev.of_node,
						 "qcom,n5v-gpio", 0);
	if (!gpio_is_valid(ctrl_pdata->n5v_gpio)) {
		pr_err("%s:%d, n5v gpio not specified\n",
						__func__, __LINE__);
	} else {
		rc = gpio_request(ctrl_pdata->n5v_gpio, "disp_n5v");
		if (rc) {
			pr_err("request reset gpio failed, rc=%d\n",
				rc);
			gpio_free(ctrl_pdata->rst_gpio);
			gpio_free(ctrl_pdata->p5v_gpio);
			return -ENODEV;
		}
		if (cont_splash_enabled)
			gpio_direction_output(ctrl_pdata->n5v_gpio, 1);
		else
			gpio_direction_output(ctrl_pdata->n5v_gpio, 0);
	}
#endif
	...
	if (rc) {
		...
		if (ctrl_pdata->rst_gpio)
			gpio_free(ctrl_pdata->rst_gpio);
#ifdef CONFIG_FB_MSM_MDSS_MIPI_DSI_INNOLUX
		if (ctrl_pdata->p5v_gpio)
			gpio_free(ctrl_pdata->p5v_gpio);
		if (ctrl_pdata->n5v_gpio)
			gpio_free(ctrl_pdata->n5v_gpio);
#endif
#ifndef CONFIG_MACH_ACER_A12
		if (gpio_is_valid(ctrl_pdata->disp_en_gpio))
			gpio_free(ctrl_pdata->disp_en_gpio);
