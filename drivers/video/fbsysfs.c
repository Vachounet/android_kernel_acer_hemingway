...
#ifdef CONFIG_ARCH_ACER_MSM8974
#ifndef CABC_DEBUG
extern int set_eco_mode(struct mdss_panel_data *pdata, bool enable) __attribute__((weak));
extern int get_eco_mode(void) __attribute__((weak));
#else
extern int set_eco_mode(struct mdss_panel_data *pdata, int mode) __attribute__((weak));
extern int get_eco_mode(void) __attribute__((weak));
#endif
...
#ifndef CABC_DEBUG
		if (tmp)
			set_eco_mode(pdata, true);
		else
			set_eco_mode(pdata, false);
#else
		set_eco_mode(pdata, tmp);
#endif
	} else
		pr_err("%s: no cabc function", __func__);
	return count;
}
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (get_eco_mode) {
		return sprintf(buf, "%d", get_eco_mode());
	} else
		return sprintf(buf, "no eco display function\n");
}
#endif
