	...
	do {
		...
		case MMC_BLK_RETRY:
			if (retry++ < 5)
				...
				/*
				 * If SD card does not support CMD23, disable it and
				 * try again
				 */
				if((card->host->index == 1) && (md->flags & MMC_BLK_CMD23)
					&& !(card->quirks & MMC_QUIRK_BLK_NO_CMD23)
					&& (brq->sbc.opcode == MMC_SET_BLOCK_COUNT)) {
					add_quirk(card, MMC_QUIRK_BLK_NO_CMD23);
					retry = 0;
					break;
				}
			}
...
static int mmc_blk_probe(struct mmc_card *card)
{
	...
	/* Increase hynix eMMC write performance */
	if (mmc_card_mmc(card))
		md->disk->queue->limits.max_sectors = 256;
#endif
