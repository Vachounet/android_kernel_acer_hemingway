...
static int mmc_read_ext_csd(struct mmc_card *card, u8 *ext_csd)
{
	...
	ext_csd_revision = ext_csd[EXT_CSD_REV];
#endif
out:
	return err;
}
...
static int mmc_change_bus_speed(struct mmc_host *host, unsigned long *freq)
{
	...
	if (mmc_card_hs200(card) && card->host->ops->execute_tuning) {
		...
		while (retry > 0) {
			...
			if (err) {
				retry = retry -1;
				printk("%s - %s: tuning failed,retry=%d,freq=%d\n",
					mmc_hostname(card->host),__func__,retry,host->ios.clock);
			} else {
				break;
			}
		}
		if (err) {
			...
			/* always set to 52M if tuning failed */
			printk("%s tuning failed, set to 52M\n",__func__);
