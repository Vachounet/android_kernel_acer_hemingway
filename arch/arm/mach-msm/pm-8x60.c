...
#ifdef CONFIG_ARCH_ACER_MSM8974
static bool avoid_power_collapse;
 /*
  *Temporarily workaround for mdp iommu page issue
  * avoid power collapse when issue happen
  * release lock when panel off
  */
void dis_cpu_power_collapse(bool on_off)
{
	if (avoid_power_collapse || on_off)
		pr_err("%s: set avoid power collapse to %d", __func__, on_off);
	avoid_power_collapse = on_off;
}
#endif
