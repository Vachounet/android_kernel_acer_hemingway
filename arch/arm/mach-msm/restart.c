...
#ifdef CONFIG_ARCH_ACER_MSM8974
static int silent_reset_status = 1;
module_param(silent_reset_status, int, 0444);

static int __init silent_reset_status_setup(char *count)
{
	silent_reset_status = simple_strtol(count, NULL, 10);

	if(silent_reset_status == 0){
		download_mode = 1;
		set_restart_level(RESET_SOC);
	}

	if(silent_reset_status == 1){
		download_mode = 0;
		set_restart_level(RESET_SUBSYS_COUPLED);
	}
	return 0;
}
__setup("SilentResetStatus=", silent_reset_status_setup);
#endif
...
static void msm_restart_prepare(const char *cmd)
{
	...
	if (cmd != NULL) {
		...
		} else if (!strncmp(cmd, "adb_on", 6)) {
			__raw_writel(0x776655dc, restart_reason);
		} else if (!strncmp(cmd, "adb_off", 7)) {
			__raw_writel(0x776655db, restart_reason);
#endif
		} else {
	...
	outer_flush_all();
#ifdef CONFIG_ARCH_ACER_MSM8974
	mdelay(1000);	//delay for debug message
#endif
}
