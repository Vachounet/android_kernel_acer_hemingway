...
unsigned long __init reserve_memory_for_log(char *name,
	unsigned long mem_size)
{
	...
	mem_phys = memblock_find_in_size(mem_size) - mem_size;
	ret = memblock_remove(mem_phys, mem_size);
	BUG_ON(ret);

	pr_info("%s mem start %lx size %lx\n", name, mem_phys, mem_size);
	return mem_phys;
}
#endif
