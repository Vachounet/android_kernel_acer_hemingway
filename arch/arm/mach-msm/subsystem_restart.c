...
int modem_restart_count = 0;
static int restart_level = RESET_SUBSYS_COUPLED;

int set_restart_level(int level)
{
	restart_level = level;
	return 0;
}
EXPORT_SYMBOL(set_restart_level);
...
static int restart_level_set(const char *val, struct kernel_param *kp)
{
	...
	case RESET_SUBSYS_COUPLED:
		 pr_info("Phase [%s] behavior activated.\n", restart_levels[restart_level]);
	break;
...
module_param_call(restart_level, restart_level_set, param_get_int, &restart_level, 0644);

static int a12_reset_modem(const char *val, struct kernel_param *kp)
{
	int status = simple_strtol(val, NULL, 10);

	if(restart_level != RESET_SUBSYS_COUPLED){
		printk("%s(): restart level != RESET_SUBSYS_COUPLED, abort!\n", __func__);
		return -EINVAL;
	}
...
module_param_call(a12_reset_modem, a12_reset_modem, NULL, NULL, 0664);
#endif
...
int subsystem_restart_dev(struct subsys_device *dev)
{
	...
	if (!strcmp(dev->desc->name, "modem")) {
		modem_restart_count++;
		pr_info("[acer] modem restart count: %d\n", modem_restart_count);
	}
#endif
