...
struct ntrig_spi_platform_data {
	...
	int data_oe_gpio;
	/**
	 *  the gpio line used as "power output enable", which
	 *  convert the power from 1.8v to 3.3v to the digitizer.
	 */
	int power_oe_gpio;
	/**
     *  the gpio line used as "interrupt output enable", which
     *  convert the cpu INT from 1.8v to 3.3v to the digitizer.
	 */
	int int_oe_gpio;
