...
ifeq (Acer, $(PRODUCT_MANUFACTURER))
DTS_NAMES ?= $(shell $(PERL) -e 'while (<>) {if (/CONFIG_ARCH_ACER_((?:MSM|QSD|MPQ)[a-zA-Z0-9]+)=y/) { $$a = $$1; $$m = acer; }; $$r = $$1 if /CONFIG_MSM_SOC_REV_(?!NONE)(\w+)=y/; $$arch = $$arch.lc("$$a$$r-$$m ") if /CONFIG_ARCH_ACER_((?:MSM|QSD|MPQ)[a-zA-Z0-9]+)=y/} print $$arch;' $(KERNEL_CONFIG))
else
